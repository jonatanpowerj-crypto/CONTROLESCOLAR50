...
function semilla(){
  ...
  return {
    plantel:{nombre:'Preparatoria No. 50', universidad:'Universidad Autónoma de Guerrero', ciclo:'2026-A', toleranciaMin:10},
    docentes:[
...
let DB = JSON.parse(localStorage.getItem(CLAVE) || 'null') || semilla();
const guardar = () => localStorage.setItem(CLAVE, JSON.stringify(DB));
guardar();
...
function marcarAsistencia(alumnoId, estado, metodo){
  ...
  else DB.asistencias.push({id:uid(), fecha:sesionPase.fecha, materiaId:sesionPase.materiaId,
...
function guardarCalificaciones(){
  document.querySelectorAll('#tablaCal tbody tr').forEach(tr=>{
    ...
    tr.querySelectorAll('input').forEach(inp=>{
      ...
      if(inp.value===''){ if(existente) DB.calificaciones = DB.calificaciones.filter(c=>c!==existente); return; }
      const valor = Math.min(10, Math.max(0, +inp.value));
      if(existente) existente.valor = valor;
      else DB.calificaciones.push({id:uid(), alumnoId, materiaId:sesionCal.materiaId, parcial:sesionCal.parcial, rubro, valor});
    });
  });
  guardar(); toast('Calificaciones guardadas.');
}
...
function vistaDocentes(el){
  ...
  const pintar = ()=>{
    ...
    $('#listaDoc').querySelectorAll('[data-el]').forEach(b=>b.addEventListener('click',()=>{
      ...
      DB.docentes = DB.docentes.filter(d=>d.id!==b.dataset.el);
      DB.materias.forEach(m=>{ if(m.docenteId===b.dataset.el) m.docenteId=null; });
      guardar(); pintar(); toast('Docente dado de baja.');
    }));
  };
...
function formDocente(id){
  ...
  body=>{
    ...
    body.querySelector('#fOk').addEventListener('click', ()=>{
      ...
      if(id) Object.assign(docente(id), datos);
      else DB.docentes.push({id:uid(), ...datos});
      guardar(); cerrarModal(); render(); toast('Docente guardado.');
    });
  });
}
...
function vistaMaterias(el){
  ...
  const pintar = ()=>{
    ...
    $('#listaMat').querySelectorAll('[data-el]').forEach(b=>b.addEventListener('click',()=>{
      ...
      DB.materias = DB.materias.filter(m=>m.id!==b.dataset.el);
      DB.horarios = DB.horarios.filter(h=>h.materiaId!==b.dataset.el);
      guardar(); pintar(); toast('Materia eliminada.');
    }));
  };
...
function formMateria(id){
  ...
  body=>{
    ...
    body.querySelector('#fOk').addEventListener('click', ()=>{
      ...
      if(id) Object.assign(materia(id), datos);
      else DB.materias.push({id:uid(), ...datos});
      guardar(); cerrarModal(); render(); toast('Materia guardada.');
    });
  });
}
...
function vistaGrupos(el){
  ...
  $('#nuevoGrupo').addEventListener('click', ()=>{
    ...
    body=>{
      ...
      body.querySelector('#gOk').addEventListener('click', ()=>{
        ...
        guardar(); cerrarModal(); render(); toast('Grupo creado.');
      });
    });
  });
  $('#bajaGrupo').addEventListener('click', ()=>{
    ...
    guardar(); render(); toast('Grupo eliminado.');
  });
  ...
  const pintar = ()=>{
    ...
    $('#listaAl').querySelectorAll('[data-el]').forEach(b=>b.addEventListener('click',()=>{
      ...
      guardar(); pintar(); toast('Alumno dado de baja.');
    }));
  };
  pintar();
}
function formAlumno(id){
  ...
  body=>{
    ...
    body.querySelector('#aOk').addEventListener('click', ()=>{
      ...
      if(id) Object.assign(alumno(id), datos);
      else DB.alumnos.push({id:uid(), ...datos});
      guardar(); cerrarModal(); render(); toast('Alumno guardado.');
    });
  });
}
...
function vistaHorarios(el){
  ...
  $('#nuevoHor').addEventListener('click', ()=>{
    ...
    body=>{
      ...
      body.querySelector('#hOk').addEventListener('click', ()=>{
        ...
        DB.horarios.push({id:uid(), materiaId:body.querySelector('#hMat').value, grupoId:horGrupo,
          dia, hi, hf, aula:body.querySelector('#hAula').value.trim()||'—'});
        guardar(); cerrarModal(); render(); toast('Clase agregada al horario.');
      });
    });
  });
  ...
  const pintar = ()=>{
    ...
    $('#horZona').querySelectorAll('[data-del]').forEach(b=>b.addEventListener('click',()=>{
      ...
      guardar(); pintar(); toast('Clase retirada del horario.');
    }));
  };
  pintar();
}
...
function vistaReportes(el){
  ...
  <div class="card" style="margin-top:1rem">
    ...
    <p class="muted">Genera un respaldo completo (JSON) desde la barra lateral, o reinicia el sistema con datos de demostración.</p>
    <button class="btn btn-danger" id="reiniciar" style="margin-top:.6rem">Restablecer con datos de demostración</button>
  </div>`;
  ...
  $('#reiniciar').addEventListener('click', ()=>{
    if(!confirm('Esto borra TODOS los datos actuales y carga los de demostración. ¿Continuar?')) return;
    DB = semilla(); guardar(); render(); toast('Sistema restablecido con datos de demostración.');
  });
}
...
$('#fileRestaurar').addEventListener('change', e=>{
  ...
  r.onload = ()=>{
    try{
      ...
      DB = datos; guardar(); render(); toast('Respaldo restaurado correctamente.');
...
function encabezadoPDF(doc, subtitulo){
  ...
  doc.text(DB.plantel.universidad + ' · Acapulco, Gro.', 14, 17);
...
function modalImportarExcel(){
  ...
  body=>{
    ...
    body.querySelector('#impOk').addEventListener('click', ()=>{
      ...
      nuevos.forEach(c=>DB.alumnos.push({id:uid(), grupoId:gid, ...c.datos}));
      guardar(); grupoSel = gid; cerrarModal(); render();
